...
treeJSON = d3.json("graph_oct17_from_matrix.json", function(error, json) {
  var clickedElemList = [];
  ...
  var globaldomains = ["for what purpose","data required","tasks","stakeholders"];
  ...
  */
  ...
  function getParent(nodeindex, nodes, treelinks) {
    ...
    console.log("nodes[nodeindex]:", nodes[nodeindex]);
    console.log("nodeindex:", nodeindex);*/
  	var parent = null;
  	for (j = 0; j < treelinks.length; j++) {
  		link = treelinks[j];
  		source = getNodeIndexByName(link.source);
      //console.log("sourcenode index:", source);
  		target = getNodeIndexByName(link.target);
      //console.log("targetnode index:", target);
  		if (source == nodeindex) {
  			//console.log(link);
  ...
  function calcDomainPositionsX(domarray) {
    ...
    for (var j = 0; j < domarray.length; j++) {
      ...
      if (j + 1 < domarray.length) {
        domarray[j+1].startX = maxlevel * distX * (j + 1) + longestword * pixelmultiplier;
      }
    }
    ...
    /*for (var j = 1; j < domarray.length ; j++) {
      ...
      console.log("j-1:", j-1, "domarray[j-1].startX:", domarray[j-1].startX);
    }*/
  ...
  function initializeNodes(node, depth, domainindex, prevdomainmaxlevel) {
    ...
    //node.positionX = depth + prevdomainmaxlevel*distX*domainindex + getLongWordLength(flat_domarray[domainindex]) * pixelmultiplier;
  ...
  /*svgGroup.append("div")
	...
	var distY = 30;
	var distX = 260; //separation between levels within one domain
